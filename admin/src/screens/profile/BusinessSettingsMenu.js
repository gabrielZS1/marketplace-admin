...
export default function BusinessSettingsMenu({ navigation }) {
  const [open, setOpen] = useState(false);

  const overlayOpacity =
    useRef(new Animated.Value(0)).current;

  const optionsTranslateY =
          ...
          {OPTIONS.map((option) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={`mb-3 flex-row items-center justify-center rounded-full px-5 py-3 ${
                  option.dark
                    ? "bg-neutral-800"
                    : "bg-white"
                }`}
